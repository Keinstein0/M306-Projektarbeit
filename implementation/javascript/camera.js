...
export class SmoothCamera {
    ...
    update() {
        const x = this.target.x + this.radius * Math.cos(this.rotationX) * Math.sin(this.rotationY);
        const y = this.target.y + this.radius * Math.sin(this.rotationX);
        const z = this.target.z + this.radius * Math.cos(this.rotationX) * Math.cos(this.rotationY);
